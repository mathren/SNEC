subroutine output_all(modeflag)
  ...
  if (modeflag == 1) then
    ...
    call hdf5_append_1d(file_id, "fields/ye", ye(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/press", p(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/cs2", cs2(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/Q", Q(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/eps", eps(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/temp", temp(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/lum", lum(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/tau", tau(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/delta_time", delta_time(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/radius", r(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/kappa", kappa(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/kappa_table", kappa_table(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/logR_op", logR_op(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/logT", logT(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/p_rad", p_rad(1:imax),imax)
    call hdf5_append_1d(file_id, "fields/free_electron_frac", &
                                 free_electron_frac(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/E_shell", E_shell(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/time_diff", time_diff(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/time_exp", time_exp(1:imax), imax)
    call hdf5_append_1d(file_id, "fields/photosphere_tracer", &
                                 photosphere_tracer(1:imax), imax)

    ! Ion fractions
    call hdf5_append_1d(file_id, "fields/He_1", &
                        ion_fractions(He_number,1,1:imax), imax)
    call hdf5_append_1d(file_id, "fields/He_2", &
                        ion_fractions(He_number,2,1:imax), imax)
    call hdf5_append_1d(file_id, "fields/He_3", &
                        ion_fractions(He_number,3,1:imax), imax)
    call hdf5_append_1d(file_id, "fields/H_1",  &
...
subroutine hdf5_append_1d(file_id, dsetname, data, n)
  use hdf5
  implicit none
  ...
  integer(HSIZE_T) :: cur_dims(2), max_dims(2), new_dims(2)
  integer(HSIZE_T) :: chunk(2), one_row(2), offset(2)
  integer          :: hdferr
  logical          :: exists
  ...
  if (.not. exists) then
    cur_dims(1) = 1
    cur_dims(2) = n
    call h5screate_simple_f(2, cur_dims, dspace_id, hdferr, max_dims)
    call h5pcreate_f(H5P_DATASET_CREATE_F, plist_id, hdferr)
    call h5pset_chunk_f(plist_id, 2, chunk, hdferr)
    call h5dcreate_f(file_id, dsetname, H5T_NATIVE_DOUBLE, dspace_id, &
                     dset_id, hdferr, plist_id)
    offset(1) = 0
    offset(2) = 0
    call h5screate_simple_f(2, one_row, memspace_id, hdferr)
    call h5dget_space_f(dset_id, dspace_id, hdferr)
    call h5sselect_hyperslab_f(dspace_id, H5S_SELECT_SET_F, offset, one_row, hdferr)
    call h5dwrite_f(dset_id, H5T_NATIVE_DOUBLE, data, one_row, hdferr, &
